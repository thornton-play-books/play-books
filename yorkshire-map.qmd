...
The map shows places in Yorkshire that were mentioned by Alice Thornton in her *Books*. 

Red pins mark places where Thornton lived for significant periods. Hover over pins to see place name labels. The table below the map provides links to related material in a number of online resources.

To filter place names click/tap in the **Find places** box and select from the drop down menu (or start typing names and hit enter to autocomplete). Multiple selections are allowed.
...
(You can also highlight a single place by clicking on the place name in the table (click again to reset) but note that a limitation of the map is that it will not automatically move to show the highlighted place.)
...
atb_homes <- 
  c("Hips_022", "Kirk_131", "Snap_047", "StNi_050", "Oswa_041", "East_019") |>
...
atb_places_wikidata_xlsx |>
  ...
  left_join(atb_homes |> select(place_id, category), by="place_id") |>
...
atb_yorkshire |>
  ...
  select(place_id, address, wikipedia, natHeritage, visionBritain, bhoVCH, wikidata, lat, long, category)
...
bscols(widths = c(12,12,12),
  ...
  leaflet(shared_yorkshire, width = "100%") |> 
    ...
    addAwesomeMarkers(lat = ~lat, lng=~long, 
                      label = ~address, 
                      #popup = ~category,  # yes you can have both label and popup
                      icon=icons),
  ...
  datatable(shared_yorkshire,
          ...
          options = list(
            columnDefs = list(list(visible=FALSE, targets=c("place_id", "lat", "long", "category")) ), # hide named columns
